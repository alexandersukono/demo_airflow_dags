...
def glo_province_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_province,tablename)
    print(fexec.get_result())    

def glo_province(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT province FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(province) \
        VALUES(%s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_province,'glo_province')
    print(fexec.get_result())