...
def glo_r15_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r15,tablename)
    print(fexec.get_result())    
    
def glo_r15(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cgender, tgender FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cgender, tgender) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r15,'glo_r15')
    print(fexec.get_result())