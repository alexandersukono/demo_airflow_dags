...
def glo_worktype_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_worktype,tablename)
    print(fexec.get_result())    
    
def glo_worktype(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, lob, type, stat, username, tglsek FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, lob, type, stat, username, tglsek) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_worktype,'glo_worktype')
    print(fexec.get_result())