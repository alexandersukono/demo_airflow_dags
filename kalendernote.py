...
def kalendernote_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalendernote,tablename)
    print(fexec.get_result())    
    
def kalendernote(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, mynoreg, empnoreg, tanggal, catatan FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, mynoreg, empnoreg, tanggal, catatan) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalendernote,'kalendernote')
    print(fexec.get_result())