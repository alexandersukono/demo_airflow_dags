...
def glo_statusemp_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_statusemp,tablename)
    print(fexec.get_result())    
    
def glo_statusemp(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT statusemp FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(statusemp) \
        VALUES(%s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_statusemp,'glo_statusemp')
    print(fexec.get_result())