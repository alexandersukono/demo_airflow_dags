...
def glo_rax_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_rax,tablename)
    print(fexec.get_result())    
    
def glo_rax(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, clocale, clanguag, fdiscont, qsortseq, tlocale FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, clocale, clanguag, fdiscont, qsortseq, tlocale) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_rax,'glo_rax')
    print(fexec.get_result())