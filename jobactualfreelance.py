...
def jobactualfreelance_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualfreelance,tablename)
    print(fexec.get_result())    
    
def jobactualfreelance(tablename):
    ...
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, noreg, tglkerja, noregistrasi, kep, lob, dept, kal, lok, sect, grup, jabatan," +
                 "head, level, superior, subdivision, subemployee, jobdesc, tglend, taxpaid, payment, idform," +
                 "form) \
        VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualfreelance,'jobactualfreelance')
    print(fexec.get_result())