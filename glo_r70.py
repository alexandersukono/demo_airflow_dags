...
def glo_r70_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r70,tablename)
    print(fexec.get_result())    
    
def glo_r70(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, ccurriso, apexrate, clanguag, fdiscont, qsortseq, tdbaseiw FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, ccurriso, apexrate, clanguag, fdiscont, qsortseq, tdbaseiw) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r70,'glo_r70')
    print(fexec.get_result())