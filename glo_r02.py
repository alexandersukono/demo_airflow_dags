...
def glo_r02_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r02,tablename)
    print(fexec.get_result())    
    
def glo_r02(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cactive, tactive FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cactive, tactive) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r02,'glo_r02')
    print(fexec.get_result())