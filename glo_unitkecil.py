...
def glo_unitkecil_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_unitkecil,tablename)
    print(fexec.get_result())    
    
def glo_unitkecil(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT tipe, name, lob FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(tipe, name, lob) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_unitkecil,'glo_unitkecil')
    print(fexec.get_result())