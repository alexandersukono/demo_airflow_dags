...
def jobactualtype_1_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualtype_1,tablename)
    print(fexec.get_result())    
    
def jobactualtype_1(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, noreg, tglcreate, tglupdate, type FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, stat, noreg, tglcreate, tglupdate, type) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualtype_1,'jobactualtype_1')
    print(fexec.get_result())