...
def kalstandby_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalstandby,tablename)
    print(fexec.get_result())    
    
def kalstandby(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, noreg, dfrom, dto, month, year, wtime FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, noreg, dfrom, dto, month, year, wtime) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalstandby,'kalstandby')
    print(fexec.get_result())