...
def glo_levelemployee_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_levelemployee,tablename)
    print(fexec.get_result())    
    
def glo_levelemployee(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, title, grade, rank FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, title, grade, rank) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_levelemployee,'glo_levelemployee')
    print(fexec.get_result())