...
def kalenderwd_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalenderwd,tablename)
    print(fexec.get_result())    
    
def kalenderwd(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, noreg, dfrom, dto, month, year, wday, ov, status FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, noreg, dfrom, dto, month, year, wday, ov, status ) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalenderwd,'kalenderwd')
    print(fexec.get_result())