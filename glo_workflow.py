...
def glo_workflow_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_workflow,tablename)
    print(fexec.get_result())    
    
def glo_workflow(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, form, appr, acting, kode, dtact, lob FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, form, appr, acting, kode, dtact, lob) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_workflow,'glo_workflow')
    print(fexec.get_result())