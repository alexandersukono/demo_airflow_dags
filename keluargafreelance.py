...
def keluargafreelance_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(keluargafreelance,tablename)
    print(fexec.get_result())    
    
def keluargafreelance(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT noreg, nama, tahun, pendidikan, pekerjaan, hubungan, jnkel FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(noreg, nama, tahun, pendidikan, pekerjaan, hubungan, jnkel) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(keluargafreelance,'keluargafreelance')
    print(fexec.get_result())