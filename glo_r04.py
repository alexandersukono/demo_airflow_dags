...
def glo_r04_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r04,tablename)
    print(fexec.get_result())    
    
def glo_r04(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cadds, tadds FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cadds, tadds) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r04,'glo_r04')
    print(fexec.get_result())