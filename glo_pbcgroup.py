...
def glo_pbcgroup_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcgroup,tablename)
    print(fexec.get_result())    
    
def glo_pbcgroup(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, grupname FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, grupname ) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcgroup,'glo_pbcgroup')
    print(fexec.get_result())