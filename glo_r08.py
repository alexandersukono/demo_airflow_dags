...
def glo_r08_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r08,tablename)
    print(fexec.get_result())    
    
def glo_r08(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, ctitleco, titleco FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, ctitleco, titleco) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r08,'glo_r08')
    print(fexec.get_result())