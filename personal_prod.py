...
def personal_prod(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT idpersonal, badge, nama, jnkel, tgllhr, tptlhr, alamat, telepon, nohp, email, agama, statnkh, status, foto, bankname, bankbranch, bankaccount, accountname, nojamsos, npwp, passid, idabsen, xtglkerja, xquitkerja, wn, wp, ktp, left(email , strpos(email, '@') - 1) as samaacountname FROM livejtiipdbms."+ tablename)
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor(config=config)
    fexec.call_async(personal_prod,'personal')
    print(fexec.get_result())