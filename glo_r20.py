...
def glo_r20_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r20,tablename)
    print(fexec.get_result())    
    
def glo_r20(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, clossiw, clanguag, fdiscont, qsortseq, tlossiw FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, clossiw, clanguag, fdiscont, qsortseq, tlossiw) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r20,'glo_r20')
    print(fexec.get_result())