...
def glo_pbcdesc_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcdesc,tablename)
    print(fexec.get_result())    
    
def glo_pbcdesc(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, idgoal, title, detail, help, pos FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, idgoal, title, detail, help, pos) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcdesc,'glo_pbcdesc')
    print(fexec.get_result())