...
def kalender_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalender,tablename)
    print(fexec.get_result())    
    
def kalender(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, tanggal, deskripsi1, deskripsi2, tahun, nama, calname, setting1, deskripsi3, event FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, tanggal, deskripsi1, deskripsi2, tahun, nama, calname, setting1, deskripsi3, event) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kalender,'kalender')
    print(fexec.get_result())