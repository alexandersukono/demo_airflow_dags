...
def glo_r24_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r24,tablename)
    print(fexec.get_result())    
    
def glo_r24(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cpaymtyp, tpaymtyp FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cpaymtyp, tpaymtyp) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r24,'glo_r24')
    print(fexec.get_result())