...
def glo_rlokasikerja_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_rlokasikerja,tablename)
    print(fexec.get_result())    
    
def glo_rlokasikerja(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT lob, kodelokasi, alamatlokasi, stat FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(lob, kodelokasi, alamatlokasi, stat) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_rlokasikerja,'glo_rlokasikerja')
    print(fexec.get_result())