...
def glo_minggukerja_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_minggukerja,tablename)
    print(fexec.get_result())    
    
def glo_minggukerja(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, namaminggu, minggu, senin, selasa, rabu, kamis, jumat, sabtu, lob, defolt FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, namaminggu, minggu, senin, selasa, rabu, kamis, jumat, sabtu, lob, defolt) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_minggukerja,'glo_minggukerja')
    print(fexec.get_result())