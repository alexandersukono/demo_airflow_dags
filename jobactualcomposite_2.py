...
def jobactualcomposite_2_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualcomposite_2,tablename)
    print(fexec.get_result())    
    
def jobactualcomposite_2(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, noreg, tglcreate, tglupdate, tglkerja, tglend, first, second, third, fourth, composite FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, stat, noreg, tglcreate, tglupdate, tglkerja, tglend, first, second, third, fourth, composite) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualcomposite_2,'jobactualcomposite_2')
    print(fexec.get_result())