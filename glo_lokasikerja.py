...
def glo_lokasikerja_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_lokasikerja,tablename)
    print(fexec.get_result())    
    
def glo_lokasikerja(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT lob, namalokasi, alamatlokasi, kotalokasi, propinsilokasi, stat, postalcode FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(lob, namalokasi, alamatlokasi, kotalokasi, propinsilokasi, stat, postalcode) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_lokasikerja,'glo_lokasikerja')
    print(fexec.get_result())