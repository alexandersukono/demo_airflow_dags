...
def glo_r26_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r26,tablename)
    print(fexec.get_result())    
    
def glo_r26(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cvolres, cmodel, clanguag, fdiscont, qsortseq, tvolres, flatin1, uvolres FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cvolres, cmodel, clanguag, fdiscont, qsortseq, tvolres, flatin1, uvolres) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r26,'glo_r26')
    print(fexec.get_result())