...
def glo_r35_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r35,tablename)
    print(fexec.get_result())    
    
def glo_r35(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cdepttyp, clanguag, fdiscont, qsortseq, tdepttyp FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cdepttyp, clanguag, fdiscont, qsortseq, tdepttyp) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r35,'glo_r35')
    print(fexec.get_result())