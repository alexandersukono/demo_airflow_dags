...
def glo_r42_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r42,tablename)
    print(fexec.get_result())    
    
def glo_r42(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, crkloss, trkloss FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, crkloss, trkloss) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r42,'glo_r42')
    print(fexec.get_result())