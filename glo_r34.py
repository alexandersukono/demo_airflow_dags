...
def glo_r34_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r34,tablename)
    print(fexec.get_result())    
    
def glo_r34(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cwrkenv, twrkenv FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cwrkenv, twrkenv) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r34,'glo_r34')
    print(fexec.get_result())