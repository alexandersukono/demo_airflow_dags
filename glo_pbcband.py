...
def glo_pbcband_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcband,tablename)
    print(fexec.get_result())    
    
def glo_pbcband(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, periode1, periode2, efdate, status, closegoal, openresult, closeresult, permcutoff, openresult2 FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, periode1, periode2, efdate, status, closegoal, openresult, closeresult, permcutoff, openresult2) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcband,'glo_pbcband')
    print(fexec.get_result())