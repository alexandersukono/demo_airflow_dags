...
def kursus_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kursus,tablename)
    print(fexec.get_result())    
    
def kursus(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, noreg, jenis, penyelenggara, dari, ke, sertificate, nosert, description, exam FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, noreg, jenis, penyelenggara, dari, ke, sertificate, nosert, description, exam) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(kursus,'kursus')
    print(fexec.get_result())