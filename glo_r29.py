...
def glo_r29_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r29,tablename)
    print(fexec.get_result())    
    
def glo_r29(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, csalcrs, tsalcrs FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, csalcrs, tsalcrs) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r29,'glo_r29')
    print(fexec.get_result())