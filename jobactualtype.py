...
def jobactualtype_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualtype,tablename)
    print(fexec.get_result())    
    
def jobactualtype(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, jobid, stat, noreg, tglcreate, tglupdate, type FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, jobid, stat, noreg, tglcreate, tglupdate, type) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(jobactualtype,'jobactualtype')
    print(fexec.get_result())