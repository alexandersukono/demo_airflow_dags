...
def glo_rac_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_rac,tablename)
    print(fexec.get_result())    
    
def glo_rac(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT region, countrycode, ledgercode, division, majorcode, entitycode, deptcode, deptname, bizsubunit, depttype, deptcategory, fundedby, financialapprover, facode, orgcode, finqualifier, coverageid FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(region, countrycode, ledgercode, division, majorcode, entitycode, deptcode, deptname, bizsubunit, depttype, deptcategory, fundedby, financialapprover, facode, orgcode, finqualifier, coverageid) \
        VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",records)
    conn2.commit()
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_rac,'glo_rac')
    print(fexec.get_result())