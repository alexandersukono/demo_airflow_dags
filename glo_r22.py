...
def glo_r22_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r22,tablename)
    print(fexec.get_result())    
    
def glo_r22(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cmgrlviw, clanguag, fdiscont, qsortseq, tmgrlviw FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cmgrlviw, clanguag, fdiscont, qsortseq, tmgrlviw ) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r22,'glo_r22')
    print(fexec.get_result())