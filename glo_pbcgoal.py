...
def glo_pbcgoal_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcgoal,tablename)
    print(fexec.get_result())    
    
def glo_pbcgoal(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, idband, band, title, detail, help, title2, detail2, help2, title3, detail3, help3 FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, stat, created, createdby, ipcrea, updated, updatedby, ipupd, idband, band, title, detail, help, title2, detail2, help2, title3, detail3, help3) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_pbcgoal,'glo_pbcgoal')
    print(fexec.get_result())