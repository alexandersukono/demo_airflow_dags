...
def glo_r13_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r13,tablename)
    print(fexec.get_result())    
    
def glo_r13(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cempltyp, cregtemp, cfullprt, templtyp FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cempltyp, cregtemp, cfullprt, templtyp) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r13,'glo_r13')
    print(fexec.get_result())