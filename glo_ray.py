...
def glo_ray_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_ray,tablename)
    print(fexec.get_result())    
    
def glo_ray(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cwwposit, cjobfam, csalbiw, csalbhr, twwptitl, twwposit, cjobc1iw, cjobc2iw FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cwwposit, cjobfam, csalbiw, csalbhr, twwptitl, twwposit, cjobc1iw, cjobc2iw) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_ray,'glo_ray')
    print(fexec.get_result())