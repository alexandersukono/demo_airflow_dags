...
def glo_taxjamsos_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_taxjamsos,tablename)
    print(fexec.get_result())    
    
def glo_taxjamsos(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, company, address, npwp, jamsostek, location, name, tipex FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, company, address, npwp, jamsostek, location, name, tipex) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_taxjamsos,'glo_taxjamsos')
    print(fexec.get_result())