...
def glo_ump_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_ump,tablename)
    print(fexec.get_result())    
    
def glo_ump(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT id, province, city, ump, stat, tahun, bulan, idlok FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(id, province, city, ump, stat, tahun, bulan, idlok) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_ump,'glo_ump')
    print(fexec.get_result())