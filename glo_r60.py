...
def glo_r60_function(tablename):
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r60,tablename)
    print(fexec.get_result())    
    
def glo_r60(tablename):
    ...
    cur1 = conn1.cursor()
    cur1.execute("SELECT cmodel, cisoctry, clanguag, fdiscont, qsortseq, tisoctry FROM "+ tablename)
    ...
    cur2 = conn2.cursor()
    cur2.executemany("INSERT INTO livejtiipdbms." +tablename+ "(cmodel, cisoctry, clanguag, fdiscont, qsortseq, tisoctry) \
...
if __name__ == '__main__':
    fexec = lithops.FunctionExecutor()
    fexec.call_async(glo_r60,'glo_r60')
    print(fexec.get_result())